...
class VeicoloTrasporto(ABC):
    ...
    def carica(self, peso):
        if self._carico_attuale + peso <= self._peso_massimo:
            ...
            print(f"Caricati {peso}kg sulla targa {self._targa}.")
        else:
            print(f"Capacità insufficiente per caricare {peso}kg.")

    def scarica(self):
        self._carico_attuale = 0
        print("Veicolo scaricato.")
...
class Camion(VeicoloTrasporto):
    ...
    def costo_manutenzione(self):
        # Calcolo interno senza parametri esterni
        return (self._numero_assi * 100) +  self._peso_massimo

# Test corretto
camion = Camion("AA123BB", 5000, 4)
print(f"Costo manutenzione camion: {camion.costo_manutenzione()}€")